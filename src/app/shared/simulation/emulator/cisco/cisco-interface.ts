...
export class CiscoInterface extends EmulatedInterfaceBase implements ICiscoInterface {
  onPeerStatusChanged(peerStatus: string): void {
    ...
    if(peerStatus === 'up') {
      ...
      if(this.model.status !== 'admin down') newStatus = 'down';
...
`;
  ...
  noShutdown() {
    if(!this.connection || ! this.connection.peer) return;
  ...
  shutdown() {
    ...
    if(this.model.status === 'admin down') {
      ...
      if(peerStatus === 'admin down') newStatus = 'admin down';
    } else if(this.model.status === 'up') {
      if(peerStatus === 'up') newStatus = 'admin down';
    }
    this.changeStatus(newStatus);
  }
