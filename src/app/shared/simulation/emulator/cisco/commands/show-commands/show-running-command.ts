...
const interfaceSpecificTemplate = `{{#showInterface}}Building configuration...
...
const template = `Building configuration... \n \n
...
export class ShowRunningCommand {
        ...
        handler: (cmdContext: CiscoCommandContext, cmdState: CommandState) => {
            ...
            try {
                let rendered: string = Mustache.render(template, {
                    model: cmdContext.device.model,
                    device: cmdContext.device,
                    ports: cmdContext.device.property('interfaces').filter((value: Port) => value instanceof Port).sort((a:
                        Port, b: Port) => a.id - b.id),
                        ...
                        return (text: string, render: any) => {
                            ...
                            if (vtpMode) {
                                ...
                                if (vtpMode === 'off' || vtpMode === 'transparent') {
                                    const vlans = device.model.vlans.filter((vlan) => 
                                    ...
                                    if (vlanIdsWithDefaultName.length > 0) {
                                        const normalized = CiscoFormatters.normalizeNumberSet(vlanIdsWithDefaultName);
                                        data += `\nvlan ${normalized}\n!`;
                                    }
                                }
                            }
    ...
    static showSectionSimple(cmdContext: CiscoCommandContext, text: string, render: any, selector: string | string[]): any {
        let data = '';
        const value = cmdContext.device.property(selector);
        if (value) {
            data = render(text);
        }
        return data;
    ...
    static showInterfaceSection(cmdContext: CiscoCommandContext, text: string, render: any, predicate: (value: any) => boolean) {
        ...
        const foundInterface = cmdContext.device.property('interfaces').find((value: any) =>
    ...
    static createShowChannelGroupDelegate(cmdContext: CiscoCommandContext, cmdState: CommandState) {
        return function(text: string, render: any) {
            ...
            if (name) {
                const foundInterface: GigabitEthernet = ShowRunningCommand.findInterface(cmdContext, name);
                if (foundInterface) {
                    const interfaceChanges: GigabitEthernet = foundInterface.serialize(true);
                    if (Object.keys(interfaceChanges).length > 0) {
                        const channelGroup: ChannelGroup = interfaceChanges.channelGroup;
    ...
    static createShowSpanningTreeDelegate(cmdContext: CiscoCommandContext, cmdState: CommandState) {
        return function(text: string, render: any ) {
            ...
            if (device.model.vlans) {
                const noSpanningTreeVlanIds: number[] = [];
                const spanningTreeVlanPriorityIds: { [priority: number]: number[]; } = { };
                device.model.vlans/*.filter((vlan) => 
                    ...
                    const vlanChanges: Vlan = vlan.serialize(true);
                ...
                if (noSpanningTreeVlanIds.length > 0) {
                    const normalizedSet = CiscoFormatters.normalizeNumberSet(noSpanningTreeVlanIds);
                ...
                if (Object.keys(spanningTreeVlanPriorityIds).length > 0) {
                    ...
                    priorities.forEach((priority) => {
                        const vlanIds = spanningTreeVlanPriorityIds[parseInt(priority, 10)];
                        const normalizedSet = CiscoFormatters.normalizeNumberSet(vlanIds);
            ...
            if (name) {
                ...
                if (foundInterface) {
                    if (foundInterface.switchportMode.toLowerCase() === 'disabled') {
                        ...
                        const switchport: Switchport = foundInterface.switchport;
                        if (switchport) {
                            const switchportChanges: Switchport = switchport.serialize(true);
                            if (Object.keys(switchportChanges).length > 0) {                                
                                if (switchportChanges.trunkingVlans) {
                                    const trunkingVlans = CiscoFormatters.denormalizeNumberSet(switchport.trunkingVlans);
                                    const normalizedTrunkingVlans = CiscoFormatters.normalizeNumberSet(trunkingVlans);
