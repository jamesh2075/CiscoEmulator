...
export function main() {
  ...
  describe('system test: no shutdown', () => {
    it('correctly updates status according to connected peer state', () => {
      let paths = new ExamItemPaths(noShutdownTasks);
      let sim = SimulationFactory.load(SimICND2);
      let result = ExamItemTester.InvokePath(sim, paths, 0, noShutdownScoreKey);

      for (let taskId in result.taskResults) {
        let expectedPoints = noShutdownScoreKey[taskId].points; // by default expect all points to be awarded
...
let noShutdownTasks: IExamItemTasks = {
  'A': {
    devices: 'SW1',
...
let noShutdownScoreKey: ExamItemScoreKey = {
  'A': {
    points: 1,
    verify: {
      devices: 'SW1',
