...
export class PortController {
    ...
    private static findPortInterface(ciscoInterfaces: IInterface[], channelGroupId: number): IInterface {
        ...
        for (const ciscoInterface of ciscoInterfaces) {
    ...
    static getMemberInterfaces(ciscoInterfaces: ICiscoInterface[], channelGroupId: number): ICiscoInterface[] {
        const memberInterfaces: ICiscoInterface[] = [];
        for (const ciscoInterface of ciscoInterfaces) {
    ...
    private onChannelGroupChanged(data: any) {
        const cmdContext = data.cmdContext;
        const cmdState = data.cmdState;
        const ciscoInterfaces = cmdContext.device.model.interfaces;
        const channelGroup = data.channelGroup;
    ...
    private onRemovePortChannel(data: any) {
        ...
        const cmdState = data.cmdState;
        const ciscoInterfaces = cmdContext.device.model.interfaces;
        const portChannelValue = data.portChannelValue;
        ...
        for (const port of ciscoInterfaces) {
        ...
        for (const port of ciscoInterfaces) {
            if (port.type === 'Port' && port.id === portChannelValue) {
                const index = cmdContext.device.model.interfaces.indexOf(port);
    ...
    * */
    public static verifyRemoteMembers(srcInterface: ICiscoInterface): boolean {
        let isMatch = false;
        const connection = srcInterface.connection;
        ...
        switch (srcInterface.property(['channelGroup', 'protocol'])) {
            case 'PAGP':
                if (connection && connection.peer.channelGroupMode !== 'desirable') {
                    isMatch = false;
                }
                break;
            case 'LACP':
                if (connection && connection.peer.channelGroupMode !== 'active') {
                    isMatch = false;
                }
                break;
        }
        ...
        switch (srcInterface.property(['switchport', 'mode'])) {
            case 'static access':
                if (connection && connection.peer.switchportAccessVlan !== '') {
                    isMatch = false;
                }
                break;
            case 'trunk':
                if (connection && connection.peer.switchportTrunkVlan !== '') {
                    isMatch = false;
                }
                break;
        }

        const protocol = srcInterface.property(['channelGroup', 'protocol']);
        const peerProtocol = connection ? connection.peer.channelGroupProtocol : null;
        isMatch = isMatch && (protocol === peerProtocol);

        const mode = srcInterface.property(['switchport', 'mode']);
        const peerMode = connection ? connection.peer.switchportMode : null;
    ...
    public static etherChannelBundler(ciscoInterfaces: ICiscoInterface[]): any {
        const portInterfaces = PortController.getPortInterfaces(ciscoInterfaces);
        if (portInterfaces && portInterfaces.length > 0) {
            for (const portChannel of portInterfaces) {
                let portCount = 0;
                const memberInterfaces = PortController.getMemberInterfaces(ciscoInterfaces, portChannel.property('id'));
                if (memberInterfaces && memberInterfaces.length > 0) {
                    for (const memberInterface of memberInterfaces) {
                        const isMatch = PortController.verifyLocalMembers(portChannel, memberInterface) &&
                         PortController.verifyRemoteMembers(memberInterface);
                        if (isMatch) {
