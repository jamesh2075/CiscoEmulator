...
export class ExamItemSpec {
  ...
  static IsStepValid(step: IExamItemStep): boolean {
    const hasChildSteps = <boolean> (step.selectOneStep || step.steps || step.distractorSteps);
    ...
    if (hasChildSteps) {
      ...
      if (step.selectOneStep) {
        if (!ExamItemSpec.AreStepsValid(step.selectOneStep)) {
          return false;
        }
      }
      if (step.steps) {
        if (!ExamItemSpec.AreStepsValid(step.steps)) {
          return false;
        }
      }
      if (step.distractors) {
        if (!ExamItemSpec.AreStepsValid(step.distractors)) {
          return false;
        }
      }
